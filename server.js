...
const setupDB = async () => {
    ...
    try {
        ...
        `);
        // Actualizar/Crear Admin con contraseña: admin
        await client.query(`
            INSERT INTO usuarios (nombre, email, password, rol)
            VALUES ('Admin Emmanuel', 'emma2013rqgmail.com', 'admin', 'admin')
            ON CONFLICT (email) DO UPDATE SET password = 'admin', rol = 'admin';
        `);
        console.log("✅ DB y Admin (pass: admin) listos.");
...
app.post('/api/auth/register', async (req, res) => {
    ...
    try {
        const resu = await pool.query('INSERT INTO usuarios (nombre, email, password) VALUES ($1,$2,$3) RETURNING *', [nombre, email, password]);
        res.json(resu.rows[0]);
    } catch (e) { res.status(500).json({ message: "Error al registrar" }); }
});

app.post('/api/work/save', async (req, res) => {
    const { usuario_id, actividad, inicio, fin, fecha, duracion } = req.body;
    try {
        await pool.query('INSERT INTO tiempos (usuario_id, actividad, hora_inicio, hora_fin, fecha, duracion_total) VALUES ($1,$2,$3,$4,$5,$6)', 
